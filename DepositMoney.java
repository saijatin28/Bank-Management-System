...
public class DepositMoney extends JInternalFrame implements ActionListener {
	...
	public void actionPerformed (ActionEvent ae) {
		...
		//			System.out.println("Creating table in given database..."); 
					stmt = conn.createStatement(); 
					String sql;
					sql = "select deposit from data where acno='"+txtNo.getText()+"';";
					var rs = stmt.executeQuery(sql);
					int ini_dep = 0;
					if (rs.next()) {
						ini_dep = Integer.parseInt(rs.getString(1));
					...
					int add = Integer.parseInt(txtDeposit.getText());
					sql = "update data set deposit='"+Integer.toString(add+ini_dep)+"' where acno='"+txtNo.getText()+"';";
