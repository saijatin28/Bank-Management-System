...
public class ViewOne extends JInternalFrame implements ActionListener {
	...
	public void load () {
		String sql = "SELECT ROW_NUMBER() OVER (ORDER BY (SELECT '1')) AS RowID, * FROM data";
...
//			System.out.println("Creating table in given database..."); 
			...
			int i=0;
			while (rs.next()) {
				records[i][0] = rs.getString(2);
				records[i][1] = rs.getString(3);
				records[i][2] = rs.getString(4);
				records[i][3] = rs.getString(5);
				records[i][4] = rs.getString(6);
				records[i][5] = rs.getString(7);
				i++;
			}
			total = i;
			showRec(0);
